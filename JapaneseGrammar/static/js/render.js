...
window.toggleMode = function(mode) {
    ...
    // 然后只显示对应的部分
    if (mode === "beginner") {
        document.getElementById("ToggleBeginnerTitle").textContent = "初级语法";
        ...
        document.getElementById("ToggleBeginnerTitle").textContent = "中级语法";
        ...
        document.getElementById("ToggleBeginnerTitle").textContent = "高级语法";
...
// 统一调用不同级别的显示函数
window.renderGrammarDetails = function (item, level) {
  if (level === "beginner") {
    window.displayBeginner(item);
  } else if (level === "intermediate") {
    window.displayIntermediate(item);
  } else if (level === "advanced") {
    window.displayAdvanced(item);
  }
};